...
int cmd_tcpecho(int argc, char **argv)
{
    tcpecho(1);
}

FINSH_FUNCTION_EXPORT(cmd_tcpecho, startup or stop TCP echo server);
#endif
//MSH_CMD_EXPORT(cmd_tcpecho, startup or stop TCP echo server);
FINSH_FUNCTION_EXPORT_ALIAS(cmd_tcpecho, __cmd_tcpecho, ping network host);
